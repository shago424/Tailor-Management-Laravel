...
class CustomerController extends Controller
{
   ...
   $data = Payment::orderby('id','DESC')->where('due_amount', '>', '0')->latest()->get();
    return view('backend.customer.due-payment-customer',compact('data'));
    }
    ...
    public function invoicecustomeredit($invoice_id){
      ...
      public function paymentcustomeredit($id){
        $editdata = Payment::find($id);
      $payment = Payment::where('id',$id)->first();
       $invoicedetails = InvoiceDetail::where('invoice_id',$payment->invoice_id)->get();
      return view('backend.customer.edit-payment-customer-invoice',compact('payment','invoicedetails','editdata'));
    } 
     ...
     public function invoicecustomerupdate(Request $request,$invoice_id){
         ...
         $payment = Payment::where('invoice_id',$invoice_id)->first();
         $payment_details = PaymentDetail::where('invoice_id',$invoice_id)->first();
         $payment->paid_amount = $request->paid_amount;
          $payment->due_amount = $request->due_amount;
         // $payment->discount_amount = $payment->paid_amount-$request->paid_amount;
        ...
        $invoice->customer_delivery_date = $payment->payment_date;
      ...
      return redirect()->route('invoices.order-view')->with('success','Customer Invoice Payment Update Successfully');
