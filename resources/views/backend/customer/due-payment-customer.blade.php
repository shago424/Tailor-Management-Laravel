          ...
          <section class="col-md-12">
           ...
           <div class="card">
            ...
            <div class="card-body">
                ...
                <td style="text-align: center;">{{$payment->order_no}}</td>
                 <td style="text-align: center;">{{date('d-m-Y',strtotime($payment['invoice']['invoice_date']))}}</td>
                <td>{{$payment['customer']['name']}}</td>
                <td>{{$payment['customer']['mobile']}}</td>
                @php
                 // $all_amount = App\Model\paymentDetails::where('payment_id',$tailor->id)->sum('item_price');
                 @endphp
                <td style="text-align: right;">{{$payment->total_amount}}</td>
                <td style="text-align: right;">{{$payment->paid_amount}}</td>
                <td  style="text-align: right;color: red">{{$payment->due_amount}}</td>
               ...
               {{--  <td style="text-align: right;">
                    ...
                    @endif
                      ...
                      <td width="12%" style="text-align: center;">
                          <a href="{{route('invoices.allview',$payment->id)}}" class="btn btn-primary btn-xs"  title="Show All Data"> <i class="fa fa-eye"></i></a>
                           <a href="{{ route('customers.invoice-payment-edit',$payment->id) }}" class="btn btn-warning btn-xs"  title="Customer Payment"> <i class="fas fa-dollar-sign"></i></a>
